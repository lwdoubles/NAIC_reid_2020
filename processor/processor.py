...
def do_inference(cfg,
    ...
    for index, loader in enumerate(val_loader):
        if index == 0:
            ...
            # reranking_parameter = [20, 6, 0.4]
            reranking_parameter = [30, 6, 0.5]
            # reranking_parameter = [35, 6, 0.5]
        ...
        for n_iter, (img, pid, camid, imgpath) in enumerate(loader):
            with torch.no_grad():
                ...
                if cfg.TEST.FLIP_FEATS == 'on':
                    ...
                    # feat = torch.FloatTensor(img.size(0), 1280).zero_().cuda()
                    for i in range(2):
                        if i == 1:
    ...
    with open(os.path.join(cfg.OUTPUT_DIR, 'result_{}.json'.format("8_27_2")), 'w',encoding='utf-8') as fp:
